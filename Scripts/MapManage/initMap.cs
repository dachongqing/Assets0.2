...
public class initMap : MonoBehaviour
{
    ...
    private Dictionary<int[], int[]> mapHidden;
    ...
    private void genUpMap() {
       ...
       // Debug.Log("map count: " + map.Count);
        ...
        foreach (int[] key in mapUp.Keys)
        {
            ...
            //根据房间的高度，竖直偏移
            newMap.y = RoomConstant.ROOM_Y_UP + key[1] * vertiDis - 0.85f;
            //根据房间的楼层，设定z坐标值
            newMap.z = key[2];
            //设置新房间在屏幕上的坐标
    ...
    private void genHiddenMap()
    {
        ...
        if (neworLoad)
        {
            this.mapHidden = mapManager.genHiddenMap(RoomConstant.ROOM_Z_X, 3);
        }
        else
        {
            this.mapHidden = genMap(Application.persistentDataPath + "/Save/SaveData4.sav");
        }
        // Debug.Log("map count: " + map.Count);
        //根据地图数据，生成新房间
        foreach (int[] key in mapHidden.Keys)
        {
            //新房间在地图中的坐标
            int[] rXYZ = new int[] { key[0], key[1], key[2] };
            //产生新房间
              Debug.Log(rXYZ[0] +","+ rXYZ[1] + "," + rXYZ[2]);
            unitMap = roomManager.genRoom(rXYZ, mapHidden[key]);

            //预备给新房间的坐标
            Vector3 newMap = mapSpawnDownPoint.position;
            //根据房间的宽度，水平偏移
            newMap.x = RoomConstant.ROOM_X_X + key[0] * horizonDis - 0.85f;
            //根据房间的高度，竖直偏移
    ...
    public Dictionary<int[], int[]> getMapHiddenInfo()
    {
